...
const ACTION_PROTOCOL = (todayISO, subjNames) => [
  ...
  `- {"type":"add_event","title":"...","date":"YYYY-MM-DD","time":"HH:MM opcional","kind":"parcial|entrega|clase|estudio|evento","subject":"nombre materia opcional"} — evento de UNA fecha puntual.`,
  `- {"type":"delete_event","match":"parte del título del evento"}`,
  `- {"type":"add_recurring_event","title":"...","dows":[0-6],"from":"YYYY-MM-DD","until":"YYYY-MM-DD","time":"HH:MM opcional","kind":"...","subject":"opcional"} — evento que SE REPITE (ej: "todos los miércoles"). dows: 0=domingo,1=lunes,2=martes,3=miércoles,4=jueves,5=viernes,6=sábado. Es UNA sola serie, no crees N add_event sueltos para esto. Si el usuario no da fecha "hasta", usá 4 meses después de "from".`,
  ...
  `REGLAS: Las fechas SIEMPRE en formato YYYY-MM-DD calculadas desde hoy (${todayISO}). Si el usuario dice "mañana", "el viernes", "todos los martes", etc., convertilo vos a fecha(s) exacta(s) — NUNCA le pidas que él la calcule. Podés poner varias acciones en el array (ej: organizar la semana = varias add_task). Solo emití acciones que el usuario pidió o aprobó explícitamente — NO inventes ni agregues cosas de más. Si NO hay nada que ejecutar (solo pregunta o consejo), NO pongas el marcador. Para borrar (delete_task/delete_event/delete_recurring_event/delete_note), igual emití la acción: la app le va a pedir confirmación al usuario antes de aplicarla, así que en el texto podés decir algo como "¿Confirmás que borre X?". ${subjNames.length ? `Materias del usuario (usá estos nombres EXACTOS en "subject"): ${subjNames.join(", ")}.` : "Todavía no cargó materias."}`,
].join("\n");
