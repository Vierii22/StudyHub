...
const OCIO_STATUS = { pendiente: ["Pendiente","#e8b04e"], progreso: ["En curso","#4ec5e8"], completado: ["Completado","#3ecf9a"] };
...
/* Estrellas 1-5 — ★ amarillo / ★ gris */
const StarRating = ({ value, onChange, size = 18 }) => (
  <div className="row" style={{ gap: 1, flexShrink: 0 }}>
    {[1,2,3,4,5].map(n => (
      <span key={n}
        style={{ fontSize: size, cursor: onChange ? "pointer" : "default", color: n <= value ? "#e8b04e" : "rgba(255,255,255,.18)", lineHeight: 1, transition: "color .1s", userSelect: "none" }}
...
const OcioModal = ({ item, onClose }) => {
  ...
  return (
    <Modal title={item ? "Editar" : "Agregar"} icon="sparkles" onClose={onClose}
      footer={<><span className="link" style={{ color: item ? "#e8639b" : "var(--tx-3)" }}
      ...
      <div style={{ display: "grid", gap: 14 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          <Field label="Tipo"><select className="sel-input" style={{ width: "100%" }} value={f.type} onChange={e => up("type", e.target.value)}>
            {OCIO_TYPES.slice(1).map(([t, e]) => <option key={t} value={t}>{e} {t}</option>)}
          </select></Field>
          <Field label="Estado"><select className="sel-input" style={{ width: "100%" }} value={f.status} onChange={e => up("status", e.target.value)}>
...
const Ocio = () => {
  ...
  return (
    <div className="page page-wide">
      <PageHead title="Ocio" meta={`${data.ocio.length} ${data.ocio.length === 1 ? "entrada" : "entradas"} · ${count("completado")} completados · ${count("pendiente")} pendientes`}>
        <div className="seg">
...
export { Cocina, Finanzas, Casa, Ocio, Recetas };