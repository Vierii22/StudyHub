...
const ConfigSection = ({ theme, setTheme, onEditDash, onLogout }) => {
  ...
  const changePwd = async () => {
    ...
    setPwdLoad(true);
    const { error } = await window._supabase.auth.updateUser({ password: newPwd });
    setPwdLoad(false);
  ...
  const deleteAllData = async () => {
    ...
    Store.reset();
    if (window.SupabaseStorage) window.SupabaseStorage.removeItem("sh_data");
...
Object.assign(window, { ConfigSection, MorningModal, ConfigRow });
